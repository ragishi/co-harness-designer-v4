...
> この file は毎 session 読まれる **実行プロトコルの正本**。**2 層構成**で書く — **Part A = 共通実行プロトコル**（どの repo / project でも持つ＝移植可能）／ **Part B = co-harness-designer-v4 固有**（このプロジェクトのみ＝新 repo では差し替え）。新しい repo を起こすときは **Part A を丸ごと継承し Part B を書き換える**（V3 CLAUDE.md「§1-4 共通 / §5 固有」規約の継承）。詳細仕様は所有 root に置き、ここに複製しない。Codex など他 agent は `AGENTS.md`（→ 本 file への pointer）経由で同じ契約を読む。

## Role

You are the implementation assistant for **co-harness-designer-v4**.（新 repo では repo 名のみ差し替え）

## このファイルの 2 層構成（境界）

| Part | 層 | 含む section | 新 repo を起こすとき |
| --- | --- | --- | --- |
| **A 🌐** | 共通実行プロトコル | 作業の基準 / Phase 0–4 / 出力・完遂・Git / 出力フォーマット / プロセス規律 / Before Completion | **そのまま継承** |
| **B 📦** | co-harness-designer-v4 固有 | Mission / Absolute Rules / Operating Modes / Boot Order / 16 Root Map / 固有規律・Gate・Learn 先 | **置き換える** |

- **判定ルール** — 「別の repo（SNS / YouTube 等）でも文言そのままで意味が通るか?」→ 通るなら **A（普遍）**。V4 の構造（root / JSON 方針 / MNP / archetype / mode）を名指すなら **B（固有）**。
- **〔V4〕印** — Part A の中で V4 固有のバインディングを指す箇所に付ける（具体定義は Part B）。
- この 2 層構成は **V4 が産む target repo の CLAUDE.md にも適用**する（Part A 継承 + Part B 差替）。

---

# 🌐 Part A — 共通実行プロトコル（移植可能 / Portable）

> どの repo / project でも持つ普遍層。新 repo はこのブロックを丸ごと継承する。

## 作業の基準（Operating Standard）

全タスクに効く既定の姿勢。**通常 60 点で済ませず、既定で 100 点を狙う**（quality boost を待たず常時適用）。構造の不変条件（Part B / Absolute Rules）が「何を守るか」なら、ここは「どう働くか」。
...
- **Task-Type / Mode Lock** — 計画・編集の前に、今回が「どの種類のタスクか」を固定する。mode が定まるまで scaffold・編集を始めない。〔V4: 5 つの operation mode は Part B / Operating Modes〕
- **Permission Lock** — `read-only` / `no-edit` / `調査だけ` は編集・commit・package 作成より優先する。

### Phase 1: Observe（必要十分に状況を掴む）

対象の役割・既存 `feedback.md`・最新 git 状態・参照 repo との差分を見て、必要な read だけ行う。読みすぎて遅くしないが、根拠なしに推測しない。〔V4: 読む単位は `05_read_sets/` の該当 set〕
...
Think で決めた方針に沿って進める。途中で前提が崩れたら理由と差分を明示して計画を更新する。部分最適に寄らず、repo 全体・将来の運用負荷まで見て判断する。止まる場合は、事実・原因・進捗・次の選択肢を短く報告する。〔V4: 16 root 全体・target repo への継承も視野に〕

### Phase 4: Learn（再利用できる学びを残す）

実行後は、適切な `feedback.md` に記録する。記録は必要十分に留め、個別対応で終わる内容を過剰に上位ルール化しない。〔V4: 具体 path と昇格 gate は Part B / Learn 先〕
...
- **品質確認**: 完了前検査を正本に対して自己実行する。STUB success / 未実装を pass と誤認しない。〔V4: 正本は `CLAUDE.gate.md`、実行可能 runner は持たない＝手動自己検査〕
...
構造の不変条件（Part B / Absolute Rules）とは別の、**作業プロセス**の規律。

**MUST**

- 編集前に対象範囲の `README.md` と `feedback.md` を読む。
- 新しい file / 概念の追加は decision-record（5 question）を通す。〔V4: DRR = `00_foundation/04_decision_rules.md`〕
- 完了前に Critical Gates を自己確認する。〔V4: `CLAUDE.gate.md`〕
- 学びは適切な粒度で `feedback.md` に append する。〔V4: 横断は `30_feedback/raw/`〕
...
Critical Gates をすべて自己確認する。Stop Conditions に該当したら停止して人間 review を求める。〔V4: 正本 `CLAUDE.gate.md`〕

---

# 📦 Part B — co-harness-designer-v4 固有（新 repo では差し替え）

> このプロジェクト固有層。新 repo では本 Part を当該 repo の Mission・不変条件・navigation に置換する。
...
## Operating Modes（5 作業モード）

Phase 0 / Task-Type Lock で必ず 1 つに分類する。mode が明示されるまで scaffold・編集・target repo 適用を始めない。詳細は `00_foundation/05_operating_modes.md`。

- `new_repo` — 新しい目的別 repo を作る
- `audit_existing_repo` — 既存 repo を診断（read-only、target へ書かない）
- `retrofit_existing_repo` — 既存 repo を改修（diagnose → diff → plan → approve → fix）
- `add_companyos_interface` — 既存 repo に `.companyos/` interface を足す（業務正本を入れない）
- `improve_v4_system` — V4 自身を改善（target repo を編集しない）
...
## V4 固有規律・Gate・Learn 先

- **固有 NEVER** — planned stub を勝手に実装コード化する／ runtime 設定（`.claude/` `.codex/` `.github/`）に V4 設計正本を置く。
- **Gate 正本** — `CLAUDE.gate.md`（実行時の短縮 checklist）/ `07_quality/30_critical_gates.md`（詳細正本。矛盾時はこちらを採用）。
- **Learn 先** — tier 直下 `feedback.md`（その tier の改善ログ）/ 横断は `30_feedback/raw/{YYYY-MM}.md` / 昇格は `31_learning/` の 4-AND gate（再発 ≥ 3 / 冷却 ≥ 7 日 / grader / boundary）。
- **詳細の委譲先** — V3 継承 → `00_foundation/09_v3_to_v4_mapping.md` ／ 品質 → `07_quality/` ／ runtime tooling → `.claude/` `.codex/` `.github/`。
...
- **2 層維持**: Part A（共通）は移植可能に保つ。repo 固有事項は Part B に置く。新 repo は Part A 継承 + Part B 差替。
