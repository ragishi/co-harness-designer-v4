...
**MVP の status 扱い（移行方針）**:
- MVP では `workflow_status` を task status としても共用する
- 後段で `03_status_metrics/20_task_status.md` を作る場合は、Task entity の status pointer をそちらへ移行する
- 移行時は migration package（`../00_foundation/04_decision_rules.md`）を通す
